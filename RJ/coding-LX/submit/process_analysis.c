...
/*
 * process_analysis.c
 * Original Author:  yejunan@ruijie.com.cn, 2021-5-31
 *
 * Analyse process.
 *
 */
...
static int get_cmd_name(char *substr, char *src, char *des)
{
    int i;
    int j;
    int len;
...
static void get_command(int pid, char *comline_s)
{
    ...
    while (fgets(line, cmd_lsize, fp)) {
        if (sscanf(line, "%31[^:]", substr) == 1) {
            if (get_cmd_name(substr, line, comline_s) > 0) {
                break;
            }
        }
    }
...
/* 指定进程名并写入信息 */
static void analysis_specific_process(char *process_name, process_t *this_process, 
                int *process_num)
{
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1) {
        ...
        qsort(this_process, process_count, sizeof(process_t), compare_function);
    }
